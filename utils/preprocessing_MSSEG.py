...
if __name__ == '__main__':
    ...
    base_folder = '/home/ubuntu/Project/datasets/MSSEG2015/training/'
    train_path = '../data/MSSEG2015/train/'
    test_path = '../data/MSSEG2015/test/'

    # 获取每个患者的文件夹路径
    patient_folders = sorted(glob.glob(os.path.join(base_folder, 'training*', 'training*_??')))
    ...
    for i, patient_folder in enumerate(patient_folders):
        ...
        pd_path = os.path.join(patient_folder, 'pd_pp_registered.nii')
        t2_path = os.path.join(patient_folder, 't2_pp_registered.nii')
        flair_path = os.path.join(patient_folder, 'flair_pp_registered.nii')
        ...
        tensor = np.stack([pd_data, t2_data, flair_data, gt_data])  
